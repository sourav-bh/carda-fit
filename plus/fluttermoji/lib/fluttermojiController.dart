...
class FluttermojiController extends GetxController {
  ...
  FluttermojiController({this.image});
  ...
  void init() async {
    Map<String?, int> _tempIndexes = await getFluttermojiOptions(image: image);
  ...
  Future<void> setFluttermojiSelectedOptions(
      {required String selectedOption}) async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    Map<String?, dynamic> oldSelectedOptions;
    try {
      oldSelectedOptions = jsonDecode(selectedOption);
    } catch (e) {
      oldSelectedOptions = defaultFluttermojiOptions;
    }
    selectedOptions = Map.from(oldSelectedOptions);
    await pref.setString('fluttermojiSelectedOptions', jsonEncode(selectedOptions));
    var fluttermojiNew = getFluttermojiFromOptions();
    await pref.setString('fluttermoji', fluttermojiNew);
    fluttermoji.value = fluttermojiNew;
    update();
  }

  bool _checkIsFlutterMojiOptionMap(String value) {
    return value.contains('topType') ||
        value.contains('hairColor') ||
        value.contains('facialHairType') ||
        value.contains('facialHairColor') ||
        value.contains('clotheType') ||
        value.contains('clotheColor') ||
        value.contains('eyeType') ||
        value.contains('eyebrowType') ||
        value.contains('mouthType') ||
        value.contains('skinColor') ||
        value.contains('accessoriesType');
  }
...
<use fill="#D0C6AC" xlink:href="#path-5"></use>''' +
  ...
  Future<Map<String?, int>> getFluttermojiOptions({String? image}) async {
    if (![null, ''].contains(image)) {
      await setFluttermojiSelectedOptions(selectedOption: image!);
    }
    SharedPreferences pref = await SharedPreferences.getInstance();
...
class FlutterMojiStatic {
  static disposeController() {
    Get.delete<FluttermojiController>();
  }

  static Map<String?, dynamic> _selectedOptions = {};

  static String getFluttermojiFromOptions(
...
<use fill="#D0C6AC" xlink:href="#path-5"></use>''' +
  ...
  static String _getFluttermojiProperty(String type) {
    return fluttermojiProperties[type]!
        .property!
        .elementAt(_selectedOptions[type] as int);
  }
}