...
typedef MojiMap = Map<MojiKeyEnum, List<int>>;
...
class FluttermojiCustomizer extends StatefulWidget {
  ...
  FluttermojiCustomizer({
    ...
    Map<MojiKeyEnum, List<int>>? attributeUnlocked,
    this.autosave = true,
    this.unlockMojiFunc,
    this.unlockMojiCallback,
    this.lockWidget,
  })  : assert(
        ...
        this.attributeUnlocked = attributeUnlocked ?? attributeUnlockedItemMap,
        super(key: key);
  ...
  final Map<MojiKeyEnum, List<int>> attributeUnlocked;

  final Future<bool> Function()? unlockMojiFunc;
  final Function(MojiKeyEnum, int)? unlockMojiCallback;
...
class _FluttermojiCustomizerState extends State<FluttermojiCustomizer>
  ...
  late Map<MojiKeyEnum, List<int>> attributeUnlocked;
  ...
  void initState() {
    ...
    attributeUnlocked = widget.attributeUnlocked;
  }
    ...
    for (var attributeIndex = 0;
      ...
      var _tileGrid = GridView.builder(
        ...
        itemBuilder: (BuildContext context, int index) =>
          ...
          MojiKeyEnum mojiEnum = getMojiEnum(attribute.key!);
          var itemIsLock = attributeUnlocked[mojiEnum]?.contains(index);
          return Stack(
            children: [
              InkWell(
              ...
              if (itemIsLock == false && widget.lockWidget != null)
                Positioned.fill(
                  child: InkWell(
                    onTap: () async {
                      var result = await widget.unlockMojiFunc?.call();
                      if (result == false) return;
                      attributeUnlocked = updateUnlockedItemMap(
                          widget.attributeUnlocked, mojiEnum, [index]);
                      widget.unlockMojiCallback?.call(mojiEnum, index);
                      setState(() {});
                    },
                    child: Container(
                      decoration: _getLockDecoration(index, i),
                      child: widget.lockWidget,
                    ),
                  ),
                )
            ],
